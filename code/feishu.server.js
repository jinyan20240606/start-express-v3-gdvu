...
const FEISHU_APP_ID = process.env.APPID || "cli_a6efdcdb127a100e"; // 飞书的应用 ID
const FEISHU_APP_SECRET = process.env.SECRET || "8OrUx2wsoS5gNBvIoZlcvfvZ1402LphB"; // 飞书的应用的 Secret
const FEISHU_BOTNAME = process.env.BOTNAME || "chatbot"; // 飞书机器人的名字
const OPENAI_KEY = process.env.KEY || "sk-9f80f5cbaed0400e85fc3bf8497081de"; // OpenAI 的 Key
const OPENAI_MODEL = process.env.MODEL || "qwen-plus"; // 使用的模型
...
async function getOpenAIReply(prompt) {
  console.log('开始调用了:getOpenAIReply')
  ...
  try{
      ...
      console.log(response.data.choices[0].message.content, '188--getOpenAIReply:-----------')
      // 去除多余的换行
...
async function handleReply(userInput, sessionId, messageId, eventId) {
  console.log(userInput, sessionId, messageId, eventId, '283:handleReply:--------')
...
// handleReply({text: '你是谁？'})

/**
 * params: { encrypt, type, header, event } 飞书的req.body
 * returns: res.send type
 */
